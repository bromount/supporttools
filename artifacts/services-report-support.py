...
def sendmail_managers():
    sleep(5)
    with open("/opt/cri/html/"+timeStamp()+"_sr_details_support.html","r") as file_open:
        lines=file_open.read()
        print lines

    subjectline = "Service Request Tracker Report - Support"
...
for row in reader:
    if row[0]:
        ...
        #print data
        if counter > 0:
            html_file=open('/opt/cri/html/'+timeStamp()+'_sr_details_support.html','a')
