...
def sendmail2(attach):
    subjectline = "[IGNORE] CRI Tracker report"
    
    receiver_email = ['annamalai@collab.net']
    ...
    msg['Cc'] = ",".join(copied)
    #emails=receiver_email+copied
    print "Going to print the file"
    
    file_open=open(timeStamp()+"_cri_details_cert.html","r")
    print "File opened"
    ...
    try:
        smtpObj = smtplib.SMTP('mail-qa1.sp.collab.net')
        smtpObj.sendmail(sender, receiver_email, msg.as_string())         
        print "Successfully sent email"
    except:
        print "Error: unable to send email"
...
for row in reader:
    ...
    if counter > 0:
        html_file=open(timeStamp()+'_cri_details_cert.html','a')
